...
class SimulateDataSet:
    ...
    def from_params(
        ...
        *,
        ...
        cls.conf = {
            ...
            "img_size": (img_size,),
            "fov_center_ra": ([ra, ra],),
            "fov_center_dec": ([dec, dec],),
            "fov_size": fov,
            "corr_int_time": int_time,
            ...
            "amp_phase": amp_phase,
            "in_path": input_path,
            "out_path_fits": out_path_fits,
            "out_path_gridded": out_path_gridded,
        }

        cls.out_path = Path(cls.conf["out_path_fits"])

        if not cls.out_path.isdir():
            cls.out_path.mkdir(parents=True, exist_ok=True)
    ...
    def _run_slurm(self):
        """ """
        job_id = int(self.job_id + self.n * 500)
        out = self.out_path / Path("vis_" + str(job_id) + ".fits")

        imgs_bundle = len(open_bundles(self.data[0]))
        bundle = torch.div(job_id, imgs_bundle, rounding_mode="floor")
        image = job_id - bundle * imgs_bundle

        SI = torch.tensor(open_bundles(self.data[bundle])[image])

        if len(SI.shape) == 2:
            SI = SI.unsqueeze(0)

        obs = self.create_observation(self.conf)
    ...
    def _run(self):
        ...
        for i in tqdm(range(len(self.data))):
            SIs = self.get_images(self.data, i)

            for j, SI in enumerate(tqdm(SIs)):
                obs = create_observation(self.conf)
                vis_data = vis_loop(
                    obs, SI, noisy=self.conf["noisy"], mode=self.conf["mode"]
                )

                out = self.out_path / Path("vis_" + str(j + len(SIs) * i) + ".fits")
                hdu_list = writer.create_hdu_list(vis_data, obs)
                hdu_list.writeto(out, overwrite=True)

    def get_images(self, bundles, i) -> torch.tensor:
        """ """
        SIs = torch.tensor(open_bundles(bundles[i], key=self.key))
    ...
    def _create_observation(self) -> Observation:
        rc = create_sampling_rc(self.conf)
    ...
    def _create_sampling_rc(self):
        """
        Draw sampling options and test if at least half of the
        telescopes can see the source. If not, then new
        ...
        samp_ops = draw_sampling_opts()
        array_layout = layouts.get_array_layout(self.conf["layout"][0])
        half_telescopes = array_layout.x.shape[0] // 2

        while test_opts(samp_ops) <= half_telescopes:
            samp_ops = draw_sampling_opts(self.conf)

        return samp_ops

    def draw_sampling_opts(self):
        """
        ...
        angles_ra = np.arange(
            self.conf["fov_center_ra"][0][0],
            self.conf["fov_center_ra"][0][1],
            step=0.1,
        )
        angles_dec = np.arange(
            self.conf["fov_center_dec"][0][0],
            self.conf["fov_center_dec"][0][1],
            step=0.1,
        )
        ...
        start_time_l = datetime.strptime(
            self.conf["scan_start"][0], "%d-%m-%Y %H:%M:%S"
        )
        start_time_h = datetime.strptime(
            self.conf["scan_start"][1], "%d-%m-%Y %H:%M:%S"
        )
        ...
        scan_duration = int(
            rng.integers(self.conf["scan_duration"][0], self.conf["scan_duration"][1])
        )

        num_scans = int(
            rng.integers(self.conf["num_scans"][0], self.conf["num_scans"][1])
        )
        ...
        opts = np.array(
            [
                self.conf["mode"],
                self.conf["layout"][0],
                self.conf["img_size"][0],
                fov_center_ra,
                fov_center_dec,
                self.conf["fov_size"],
                self.conf["corr_int_time"],
                scan_start,
                scan_duration,
                num_scans,
                self.conf["scan_separation"],
                self.conf["ref_frequency"],
                self.conf["frequency_offsets"],
                self.conf["bandwidths"],
                self.conf["corrupted"],
                self.conf["device"],
                self.conf["sensitivty_cut"],
            ],
            dtype="object",
        )
    ...
    def test_opts(self, rc):
        ...
        time = self._calc_time_steps(rc)

        _, el_st_0 = self.calc_ref_elev(src_crd, time[0], array_layout)
        _, el_st_1 = self.calc_ref_elev(src_crd, time[1], array_layout)
    ...
    def calc_ref_elev(self, src_crd, time, array_layout):
        if time.shape == ():
            time = time[None]
    ...
    def _calc_time_steps(self, conf):
