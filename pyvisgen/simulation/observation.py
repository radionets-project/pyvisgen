...
class Observation:
    def __init__(
        ...
        self.interval = pointing_interval_length
        self.times = self.calc_time_steps()
        self.scans = torch.stack(
            torch.split(torch.arange(len(self.times)), (len(self.times)//self.num_scans)),
            dim=0,
            )
        ...
        self.waves_low = torch.from_numpy((const.c / ( self.IFs - self.bandwiths ) * un.second / un.meter).value)
        self.waves_high = torch.from_numpy((const.c / ( self.IFs + self.bandwiths ) * un.second / un.meter).value)
        ...
        self.calc_baselines()
        self.baselines.num = int(len(self.array.st_num) * (len(self.array.st_num) - 1) / 2)
        self.calc_valid_baselines()
        
    def calc_baselines(self):
        self.baselines = Baselines(torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]))
    ...
    def calc_time_steps(self):
        time_lst = [
            self.start + self.interval * i * un.second + j * self.int_time * un.second
    ...
    def calc_ref_elev(self, time=None):
        ...
        if time.shape == ():
            time = time[None]
        src_crd = SkyCoord(
            ra=src_ra, dec=src_dec, unit=(un.deg, un.deg)
        )
    ...
    def create_lm_grid(self):
        ...
        lm_grid[:, :, 1] = torch.sin(self.rd[:, :, 1]) * torch.cos(torch.deg2rad(self.dec)) - torch.cos(
            torch.deg2rad(self.dec)
        ) * torch.sin(np.deg2rad(self.dec)) * torch.cos(self.rd[:, :, 0] - torch.deg2rad(self.ra))
    ...
    def get_baselines(self, times):
        ...
        baselines = Baselines(torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]))
    ...
    def calc_valid_baselines(self, time=None):
        ...
        self.date = torch.repeat_interleave(
            torch.from_numpy((time[:-1] + self.int_time * un.second / 2).jd.reshape(-1, 1)),
            self.baselines.num,
            dim=1,
        )[mask]
