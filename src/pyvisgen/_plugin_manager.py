...
class PluginManager:
    ...
    def _gridding(self) -> dict[str, Callable]:
        """Classmethod that discovers all available gridding plugins.

        Returns
        -------
        dict
            Dictionary containing all available gridding plugins
            in 'pyvisgen.gridding' entry point group.
        """
        self.plugins = {}
        ...
        for entry_point in gridding_plugins:
            try:
                ...
                LOGGER.warn(f"Failed to load plugin {entry_point.name}: {e}")
